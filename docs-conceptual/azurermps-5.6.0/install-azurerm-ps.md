...
Se você quiser usar o Azure PowerShell no macOS ou no Linux, confira o seguinte artigo:  
[Instalar e configurar o Azure PowerShell em macOS e Linux](install-azurermps-maclinux.md).
...
```
Além disso, é aconselhável atualizar o PowerShellGet com o comando a seguir:
```powershell
...
|Desejo fazer uma atualização para o PowerShell 5|<ol><li>[Instalar a versão mais recente do WMF](https://www.microsoft.com/en-us/download/details.aspx?id=54616)</li><li>Execute o comando a seguir:<br/>```Install-Module PowerShellGet -Force```</li></ol>|
|Estou executando uma versão do Windows com o PowerShell 3 ou 4|<ol><il>[Obter os módulos do PackageManagement](http://go.microsoft.com/fwlink/?LinkID=746217)</il><li>Execute o comando a seguir:<br/>```Install-Module PowerShellGet -Force```</li></ol>|
